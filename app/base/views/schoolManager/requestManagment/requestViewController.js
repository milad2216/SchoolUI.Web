...
define(['app'], function (app) {
    ...
    function ($scope, $rootScope, $state, $stateParams, dataService, RESOURCES, Notification) {
        ...
        dataService.patchEntity(RESOURCES.USERS_DOMAIN + '/api/Requests/' + $scope.studentRequest.Id, { RequestState: 1 }).then(function (id) {
            if (id) {
            }
        }, function (err) {
        ...
        $scope.approvedRequest = function () {
            dataService.patchEntity(RESOURCES.USERS_DOMAIN + '/api/Requests/' + $scope.studentRequest.Id, {RequestState:2}).then(function (id) {
                if (id) {
        ...
        $scope.deniedRequest = function () {
            dataService.patchEntity(RESOURCES.USERS_DOMAIN + '/api/Requests/' + $scope.studentRequest.Id, {RequestState:3}).then(function (id) {
                if (id) {
