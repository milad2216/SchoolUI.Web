...
(function () {
    ...
    angular.module('cfp.hotkeys', []).provider('hotkeys', function () {
        ...
        this.template = '<div class="cfp-hotkeys-container fade" ng-class="{in: helpVisible}"><div class="cfp-hotkeys">' +
                        ...
                        '<div class="dialogbox">' +
                         ...
                         '<div class="dialog-Title">' + 'عملیات اصلی' + '</div>' +
                          ...
                          '<table><tbody>' +
                            '<tr ng-if="hotkey.type==3" ng-repeat="hotkey in hotkeys | filter:{ type: 3 }">' +
                              '<td class="cfp-hotkeys-keys">' +
                                '<span ng-repeat="key in hotkey.format() track by $index" class="cfp-hotkeys-key hvr-rectangle-out">{{ key }}</span>' +
                              '</td>' +
                              '<td class="cfp-hotkeys-text">{{hotkey.type==3}}{{ hotkey.description }}</td>' +
                            '</tr>' +
                          '</tbody></table>' +
                          '</div>' +
                          '</div></div>' +
        ...
        this.$get = ['$rootElement', '$rootScope', '$compile', '$window', '$document', function ($rootElement, $rootScope, $compile, $window, $document) {
             ...
             */
            function Hotkey(combo, description, callback, action, allowIn, persistent, type) {
             ...
             */
            function _add(combo, description, callback, action, allowIn, persistent, type) {
                ...
                scope.hotkeys.push(new Hotkey(combo, description, callback, action, allowIn, persistent, type));
