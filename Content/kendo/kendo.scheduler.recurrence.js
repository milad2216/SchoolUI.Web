    ...
    (function ($, undefined) {
        ...
        function toExceptionString(dates, zone) {
            ...
            for (length = result.length; idx < length; idx++) {
                date = result[idx];
                date = kendo.timezone.convert(date, zone || date.getTimezoneOffset(), 'Asia/Tehran');
        ...
        function serializeDateRule(dateRule, zone) {
            ...
            for (; idx < length; idx++) {
                val = value[idx];
                val = timezone.convert(val, tzid || zone || val.getTimezoneOffset(), 'Asia/Tehran');
        ...
        function serialize(rule, zone) {
            ...
            if (until) {
                if (until.getTime() < 0) {
                    until = (new JalaliDate(until.getFullYear(), until.getMonth(), until.getDate(), 0, 0, 0)).gregoriandate;
                }
                
                //until = until.gregoriandate || until;
                //var d = new Date(until.getTime());
                //if (d.getTime() < 0) {
                //    d = new JalaliDate(until);
                //    until = d.gregoriandate;
                //}
                until = timezone.convert(until, zone || until.getTimezoneOffset(), 'Asia/Tehran');
        ...
        var BaseRecurrenceEditor = Widget.extend({
            ...
            _initUntil: function () {
                ...
                var d = until || new Date(start.getFullYear(), start.getMonth(), start.getDate(), 23, 59, 59);
                var min = until && until < start ? until : start;
                that._until = input.kendoJalaliDatePicker({
                    min: new JalaliDate(min.getTime()),
                    value: new JalaliDate(d),
                    change: function () {
                        var date = this.value();
        ...
        var RecurrenceEditor = BaseRecurrenceEditor.extend({
            ...
            _end: function () {
                ...
                that._toggleEnd(endRule || 'until'); //todo fix this <-------
            },
            _period: function () {
                var that = this;
                var rule = that._value;
