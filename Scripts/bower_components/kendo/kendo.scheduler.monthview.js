...
(function (f, define) {
    ...
    (function ($) {
        var kendo = window.kendo, ui = kendo.ui, SchedulerView = ui.SchedulerView, NS = '.kendoMonthView', extend = $.extend, getDate = kendo.date.getDate, MS_PER_DAY = kendo.date.MS_PER_DAY, NUMBER_OF_ROWS = 6, NUMBER_OF_COLUMNS = 7, 
		//DAY_TEMPLATE = kendo.template('<span class="k-link k-nav-day">#:kendo.toString(date, "dd")#</span>'), 
		DAY_TEMPLATE = kendo.template('<span class="k-link k-nav-day" >#: new JalaliDate(date.getTime()).getDate() #</span>'), 
        ...
        ui.MonthView = SchedulerView.extend({
            ...
            dateForTitle: function () {
				var jdate = new JalaliDate(this._firstDayOfMonth.getTime());
				return kendo.getCulture().calendars.standard.months.names[jdate.getMonth()] + ' ' + jdate.getFullYear();
            ...
            _renderLayout: function (date) {
                var that = this;
                this._firstDayOfMonth = kendo_jdate.firstDayOfMonth(date);
                this._lastDayOfMonth = kendo_jdate.lastDayOfMonth(date);
            ...
            _createRow: function (startDate, startIdx, cellsPerRow, groupIndex) {
                ...
                for (var cellIdx = 0; cellIdx < cellsPerRow; cellIdx++) {
                    ...
                    if (!kendo.date.isInDateRange(startDate, min, max)) {
                        var sd = (new Date(startDate)).setHours(0, 0, 0, 0, 0),
                            md = (new Date(min)).setHours(0, 0, 0, 0, 0)
                        ;
                        if (sd !== md)
        ...
        function firstVisibleMonthDay(date, calendarInfo) {
			...
			*/
			var firstDay = calendarInfo.firstDay, 
				jFirstDay = new JalaliDate(date.getTime()),
				firstVisibleDay = new JalaliDate(jFirstDay.getFullYear(), jFirstDay.getMonth(), 0, jFirstDay.getHours(), jFirstDay.getMinutes(), jFirstDay.getSeconds(), jFirstDay.getMilliseconds()).gregoriandate;
